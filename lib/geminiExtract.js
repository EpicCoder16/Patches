...
{"groups":[{"groupTitle":"Popular videos","items":[{"title":"brief title","snippet":"one-line quote or paraphrase","importance":3,"anchorIndex":0}]}]}

Rules:
- "groups" must contain exactly one group for this user request. The group's "items" array contains 0–12 matching items; it may be empty if nothing matches.
- "groupTitle" is a concise label for the complete result set.
...
const NoteGroupSchema = z.object({
  groupTitle: z.string().trim().min(1).max(200),
  items: z.array(NotableItemSchema).max(12),
});

const ExtractResponseSchema = z.object({
  groups: z.array(NoteGroupSchema).length(1),
});
...
function filterItemsToAnchors(data, anchorCount) {
  ...
  return {
    groups: data.groups.map((group) => ({
      ...group,
      items: group.items.filter((item) => item.anchorIndex < max),
    })),
  };
}
